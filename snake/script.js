...
const SNAKE_INITIAL_SEGMENTS = 3; /* establece que los segmentos iniciales de la serpiente siempre sean 5 independemente de su posición incluyendo la cabeza */
...
const loadTop10 = function () {
    //el fetch recoge los datos de getTop10, muestra el top10 en pantalla y lo actualiza una vez se obtiene una nueva puntuación
    fetch("getTop10.php")
    .then(res => res.json())
    .then(data => {
        const div = document.querySelector("#top10");
        let html = "<h3>TOP 10</h3>";
        for (let i = 0; i < data.length; i++) {
            html += "<div>" + (i+1) + ". " + data[i] + "</div>";
            }
        div.innerHTML = html;
        });
    //el fetch recoge el numero actual de partidas jugadas y suma +1 al contador cuando se inicia una nueva partida
    fetch("getNumPartidas.php")
    .then(res => res.json())
    .then(data => {
        console.log("STATS:", data);
        const gamesDiv = document.querySelector("#gamesPlayed");
        if (gamesDiv) {
            gamesDiv.textContent = "Partidas: " + data.gamesPlayed;
        }
    });
}

const buildCellID = function(x, y){ /* esta función sirve para renombrar la variable td para que nos diga la coordenada de Row y de Column*/
    return "R" + y + "_C" + x; 
}
...
const startGame = function () {
    game.snake.speed = SNAKE_INITIAL_SPEED;
    game.applePosition = getRandomPosition();
    game.snake.segments = [];
    game.score = 0;
